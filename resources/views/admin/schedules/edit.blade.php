<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Jadwal Dr. ') }} {{ $doctor->user->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.schedules.update', ['doctor' => $doctor->id, 'schedule' => $schedule->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mt-4">
                            ...
                            <input type="time" name="start_time" id="start_time" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('start_time', date('H:i', strtotime($schedule->start_time))) }}" required>
                        </div>

                        <div class="mt-4">
                            <label for="end_time" class="block font-medium text-sm text-gray-700">Jam Selesai</label>
                            <input type="time" name="end_time" id="end_time" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('end_time', date('H:i', strtotime($schedule->end_time))) }}" required>
                        </div>

                        <div class="mt-4">
                            <label for="is_active" class="block font-medium text-sm text-gray-700">Status</label>
                            <select name="is_active" id="is_active" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                                <option value="1" {{ old('is_active', $schedule->is_active) == 1 ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('is_active', $schedule->is_active) == 0 ? 'selected' : '' }}>Nonaktif (Cuti/Libur)</option>
                            </select>
                        </div>
